...
use crate::error::{map_product_error, problem, share_link_expired_problem, ProblemDetail};
...
pub(crate) async fn health() -> Json<serde_json::Value> {
    Json(json!({ "status": "ok", "service": "sdkwork-drive-open-api" }))
}
...
pub(crate) async fn create_share_link_download_url(
    ...
    let Some(provider) = provider else {
        return Err(map_product_error(missing_signing_provider_error(&bucket)));
    };
