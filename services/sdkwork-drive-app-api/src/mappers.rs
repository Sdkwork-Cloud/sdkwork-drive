...
pub(crate) fn build_node_capabilities_response(
    ...
    inherited: bool,
    inherited_from_node_id: Option<String>,
    node_lifecycle_status: &str,
) -> NodeCapabilitiesResponse {
    let can_read = matches!(role.as_str(), "reader" | "commenter" | "writer" | "owner");
    let can_comment = matches!(role.as_str(), "commenter" | "writer" | "owner");
    let can_write = matches!(role.as_str(), "writer" | "owner");
    let can_delete = role == "owner";
    let can_manage_permissions = role == "owner";
    let can_manage_versions = matches!(role.as_str(), "writer" | "owner");
    if node_lifecycle_status == "trashed" {
