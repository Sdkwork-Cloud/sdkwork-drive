...
use crate::error::{
    internal_problem, internal_sql_error, map_object_store_route_error, map_product_error,
...
use axum::Json;
use sdkwork_drive_product::ports::storage_object_store::SignedDownloadPayload;
...
pub(crate) async fn create_download_package(
    ...
    let provider = require_active_storage_provider(provider, &bucket).map_err(map_product_error)?;
    let object_store = build_s3_object_store_for_provider(&provider)
        .await
        .map_err(map_product_error)?
        .ok_or_else(|| map_product_error(unsupported_signing_provider_error(&bucket)))?;
...
pub(crate) async fn resolve_download_package_url(
    ...
    let provider = find_storage_provider_by_id(&state.pool, &package.storage_provider_id)
        .await
        .map_err(map_product_error)?
        .ok_or_else(|| map_product_error(missing_signing_provider_error(&package.bucket)))?;
    let provider =
        require_active_storage_provider(provider, &package.bucket).map_err(map_product_error)?;
    let object_store = build_s3_object_store_for_provider(&provider)
        .await
        .map_err(map_product_error)?
        .ok_or_else(|| map_product_error(unsupported_signing_provider_error(&package.bucket)))?;
    let signed = sign_download_package(
        &object_store,
        &package.bucket,
    ...
    for file in files {
        if !stores.contains_key(&file.storage_provider_id) {
            let provider = find_storage_provider_by_id(pool, &file.storage_provider_id)
                .await
                .map_err(map_product_error)?
                .ok_or_else(|| map_product_error(missing_signing_provider_error(&file.bucket)))?;
            ...
            let object_store = build_s3_object_store_for_provider(&provider)
                ...
                .map_err(map_product_error)?
                .ok_or_else(|| {
                    map_product_error(unsupported_signing_provider_error(&file.bucket))
                })?;
