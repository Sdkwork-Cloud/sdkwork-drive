...
use crate::error::{
    internal_sql_error, map_object_store_route_error, map_product_error, not_found_problem,
...
pub(crate) async fn read_archive_node_bytes(
    ...
    let provider = find_storage_provider_by_id(&state.pool, &object_ref.storage_provider_id)
        ...
        .ok_or_else(|| map_product_error(missing_signing_provider_error(&object_ref.bucket)))?;
    let provider =
        require_active_storage_provider(provider, &object_ref.bucket).map_err(map_product_error)?;
    let object_store = build_s3_object_store_for_provider(&provider)
        .await
        .map_err(map_product_error)?
        .ok_or_else(|| map_product_error(unsupported_signing_provider_error(&object_ref.bucket)))?;
