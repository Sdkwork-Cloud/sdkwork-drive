...
impl SqlDriveWorkspaceService {
    ...
    ) -> Result<Option<DriveWorkspaceNode>, DriveProductError> {
    ...
    ) -> Result<DriveWorkspaceChildrenPage, DriveProductError> {
...
fn workspace_node_from_record(
    ...
    path: &str,
) -> Result<DriveWorkspaceNode, DriveProductError> {
...
fn parse_logical_path(path: &str) -> Result<Vec<String>, DriveProductError> {
    let path = path.trim();
    if path.is_empty() {
        return Err(DriveProductError::Validation(
...
fn require_valid_object_key(value: String) -> Result<String, DriveProductError> {
...
fn normalize_sha256_checksum(value: String) -> Result<String, DriveProductError> {
