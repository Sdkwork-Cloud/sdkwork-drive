...
use crate::{drive_share_token_hash, DriveProductError};
    ...
    S: DriveUploaderStore,
{
    ...
    pub async fn prepare_upload(
        ...
        ) = resolve_retention(command.retention, command.now_epoch_ms)?;
        ...
        let (space_id, parent_node_id) = match command.target {
            ...
            UploaderTarget::Space {
                ...
                self.ensure_target_space_upload_allowed(
                    &tenant_id,
                    &space_id,
        ...
        let (storage_provider_id, bucket) = self
            ...
            .await?
            .ok_or_else(|| {
                DriveProductError::NotFound("active storage provider not found".to_string())
            })?;
        let upload_session_id = format!("session-{id}");
        let object_key = build_uploader_object_key_prefix(
            &tenant_id,
        ...
        self.store
            .insert_upload_session(
                ...
                &object_key,
                &operator_id,
                command.now_epoch_ms + 86_400_000,
            )
            .await?;
        ...
        O: DriveObjectStore,
    {
        ...
        let upload_session_id = prepared.upload_session_id.clone().ok_or_else(|| {
            DriveProductError::Internal("prepared upload is missing upload_session_id".to_string())
        })?;
        let bucket = prepared.object_bucket.clone().ok_or_else(|| {
            DriveProductError::Internal("prepared upload is missing object bucket".to_string())
        })?;
        let object_key = prepared.object_key.clone().ok_or_else(|| {
            DriveProductError::Internal("prepared upload is missing object key".to_string())
        })?;
        ...
        object_store
            ...
            .await
            .map_err(|error| {
                DriveProductError::Internal(format!("write uploaded object failed: {error}"))
            })?;
    ...
    async fn ensure_target_space_upload_allowed(
        ...
        let space = self
            .store
            .find_active_space(tenant_id, space_id)
            .await?
            .ok_or_else(|| DriveProductError::NotFound("target space not found".to_string()))?;
        ...
        let permission_anchor_node_id = if let Some(parent_node_id) = parent_node_id {
            let parent = self
                .store
                .find_active_node(tenant_id, parent_node_id)
                .await?
                .ok_or_else(|| {
                    DriveProductError::NotFound("target parent node not found".to_string())
                })?;
            if parent.space_id != space_id || parent.node_type != "folder" {
                return Err(DriveProductError::NotFound(
        ...
        let is_space_owner = space.owner_subject_type == owner_subject_type
            && space.owner_subject_id == owner_subject_id;
        if is_space_owner && actor_type != "anonymous" {
            return Ok(());
        }
        ...
        if !permission_anchor_node_id.is_empty()
            && self
                .store
                .has_writer_permission(tenant_id, &permission_anchor_node_id, actor_type, actor_id)
                .await?
        {
            return Ok(());
        }
        ...
        if let Some(share_token) = share_token {
            if permission_anchor_node_id.is_empty() {
                return Err(DriveProductError::PermissionDenied(
        ...
        Err(DriveProductError::PermissionDenied(
...
fn resolve_actor(
    actor: UploaderActor,
    app_id: &str,
) -> Result<(String, String, Option<String>, String, String), DriveProductError> {
    match actor {
...
fn resolve_retention(
    ...
    now_epoch_ms: i64,
) -> Result<(String, Option<i64>, Option<String>, Option<i64>), DriveProductError> {
    match retention {
        UploaderRetention::LongTerm => Ok(("long_term".to_string(), None, None, None)),
        UploaderRetention::Temporary {
            ...
            let hard_delete_after_epoch_ms = hard_delete_after_seconds
                .map(|seconds| {
                    if seconds <= 0 {
                        Err(DriveProductError::Validation(
            ...
            Ok((
                "temporary".to_string(),
                Some(now_epoch_ms + ttl_seconds * 1000),
                Some(cleanup_action),
                hard_delete_after_epoch_ms,
            ))
        }
...
fn normalize_profile(value: String) -> Result<String, DriveProductError> {
...
fn require_cleanup_action(value: String) -> Result<String, DriveProductError> {
...
fn normalize_content_type(value: String) -> Result<String, DriveProductError> {
...
fn require_identifier(value: String, field_name: &str) -> Result<String, DriveProductError> {
...
fn normalize_optional_scene(value: Option<String>) -> Result<Option<String>, DriveProductError> {
...
fn require_file_name(value: String) -> Result<String, DriveProductError> {
...
fn validate_sha256_checksum(value: &str) -> Result<(), DriveProductError> {
...
#[allow(clippy::too_many_arguments)]
fn build_uploader_object_key_prefix(
    tenant_id: &str,
    ...
    let date = utc_date_from_epoch_ms(now_epoch_ms).unwrap_or("1970-01-01".to_string());
    format!(
        ...
        path_segment(organization_id.unwrap_or("none")),
        path_segment(user_id.unwrap_or("anonymous")),
        path_segment(space_id),
        path_segment(actor_type),
        path_segment(actor_id),
        path_segment(scene.unwrap_or("unspecified")),
        path_segment(source.unwrap_or("unspecified")),
        path_segment(upload_profile_code),
        date,
        path_segment(node_id),
        path_segment(upload_item_id),
    )
}
