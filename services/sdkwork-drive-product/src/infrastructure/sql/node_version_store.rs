...
impl DriveNodeVersionStore for SqlDriveNodeVersionStore {
    ...
    ) -> Result<DriveNodeVersion, DriveProductError> {
    ...
    async fn find_by_id(
        ...
        node_id: &str,
        version_id: &str,
    ) -> Result<Option<DriveNodeVersion>, DriveProductError> {
        ...
        let row = sqlx::query(&sql)
            ...
            .await
            .map_err(|error| {
                DriveProductError::Internal(format!("read dr_drive_node_version failed: {error}"))
            })?;
    ...
    async fn list_by_node(
        ...
        limit: i64,
        offset: i64,
    ) -> Result<Vec<DriveNodeVersion>, DriveProductError> {
        let rows = sqlx::query(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("list dr_drive_node_version failed: {error}"))
        })?;
...
fn map_row_to_node_version(row: &AnyRow) -> Result<DriveNodeVersion, DriveProductError> {
