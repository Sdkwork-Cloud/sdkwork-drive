    ...
    async fn find_by_idempotency(
        ...
        let row = sqlx::query(
            "SELECT id, tenant_id, space_id, node_id, bucket, object_key,
                    idempotency_key, state, expires_at_epoch_ms, version
        ...
        .map_err(|error| {
            DriveProductError::Internal(format!(
                "query drive_upload_session by idempotency failed: {error}"
            ))
        })?;
        ...
        let state = DriveUploadSessionState::try_from_str(&state_raw).ok_or_else(|| {
            DriveProductError::Internal(format!("unknown upload session state: {state_raw}"))
        })?;
    ...
    async fn insert_upload_session(
        ...
        if let Err(error) = result {
            ...
            return Err(DriveProductError::Internal(format!(
                "insert drive_upload_session failed: {message}"
            )));
        }
        ...
        .map_err(|error| {
            DriveProductError::Internal(format!(
                "read inserted drive_upload_session failed: {error}"
            ))
        })?;
