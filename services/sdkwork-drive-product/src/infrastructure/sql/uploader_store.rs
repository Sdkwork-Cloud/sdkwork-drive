...
use crate::ports::uploader_store::{
    ...
    DriveUploaderStore, NewDriveUploadItem, NewDriveUploadPart,
};
...
impl DriveUploaderStore for SqlUploaderStore {
    async fn find_upload_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find uploader upload space failed: {error}"))
        })
    }
    ...
    async fn find_active_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find uploader active space failed: {error}"))
        })?;
    ...
    async fn find_active_node(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find uploader active node failed: {error}"))
        })?;
    ...
    async fn has_writer_permission(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("check uploader writer permission failed: {error}"))
        })?;
        Ok(count > 0)
    }
    ...
    async fn insert_upload_space(
        ...
        self.find_upload_space(tenant_id, owner_subject_type, owner_subject_id, space_type)
            .await?
            .ok_or_else(|| {
                DriveProductError::Internal("uploader upload space was not created".to_string())
            })
    }
    ...
    async fn insert_upload_node(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("insert uploader node failed: {error}"))
        })?;
    ...
    async fn insert_upload_session(
        ...
        .bind(bucket)
        .bind(object_key)
        .bind(format!("uploader-{session_id}"))
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("insert uploader session failed: {error}"))
        })?;
    ...
    async fn find_default_storage_provider(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find uploader storage provider failed: {error}"))
        })?;
    ...
    async fn insert_upload_item(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("insert dr_drive_upload_item failed: {error}"))
        })?;

        self.find_upload_item_by_task(&item.tenant_id, &item.task_id)
            .await?
            .ok_or_else(|| {
                DriveProductError::Internal("inserted upload item not found".to_string())
            })
    }
    ...
    async fn find_upload_item_by_task(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find dr_drive_upload_item failed: {error}"))
        })?;
    ...
    async fn record_uploaded_part(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find dr_drive_upload_part failed: {error}"))
        })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("insert dr_drive_upload_part failed: {error}"))
        })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read dr_drive_upload_part failed: {error}"))
        })?;
...
async fn complete_stored_upload_in_transaction(
    ...
    let bucket =
        target.item.object_bucket.as_deref().ok_or_else(|| {
            DriveProductError::Internal("upload item is missing bucket".to_string())
        })?;
    let object_key = target.item.object_key.as_deref().ok_or_else(|| {
        DriveProductError::Internal("upload item is missing object key".to_string())
    })?;
    let storage_provider_id = target.item.storage_provider_id.as_deref().ok_or_else(|| {
        DriveProductError::Internal("upload item is missing storage provider".to_string())
    })?;
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("complete dr_drive_upload_item failed: {error}"))
    })?;
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("complete dr_drive_upload_session failed: {error}"))
    })?;
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("mark uploader node content ready failed: {error}"))
    })?;
...
async fn find_stored_upload_completion_target(
    connection: &mut AnyConnection,
    completion: &CompleteDriveStoredUpload,
) -> Result<StoredUploadCompletionTarget, DriveProductError> {
    let row = sqlx::query(
        "SELECT ui.*,
                us.bucket AS object_bucket,
                us.object_key AS object_key,
...
async fn read_upload_item_by_id(
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("read completed upload item failed: {error}"))
    })?;
    row.as_ref()
        .map(map_row_to_upload_item)
        .transpose()?
        .ok_or_else(|| DriveProductError::NotFound("upload item not found".to_string()))
}
...
async fn find_active_stored_upload_object(
    ...
    bucket: &str,
    object_key: &str,
) -> Result<Option<StoredUploadObject>, DriveProductError> {
    let row = sqlx::query(
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("find active stored upload object failed: {error}"))
    })?;
...
async fn insert_stored_upload_object(
    ...
    bucket: &str,
    object_key: &str,
) -> Result<StoredUploadObject, DriveProductError> {
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("insert stored upload object failed: {error}"))
    })?;
...
async fn record_upload_completed_change(
    ...
    .await
    .map_err(|error| {
        DriveProductError::Internal(format!("read stored upload change cursor failed: {error}"))
    })?;
...
fn map_row_to_upload_item(row: &AnyRow) -> Result<DriveUploadItem, DriveProductError> {
    Ok(DriveUploadItem {
        id: row.get("id"),
...
fn map_row_to_upload_part(row: &AnyRow) -> Result<DriveUploadPart, DriveProductError> {
    Ok(DriveUploadPart {
        id: row.get("id"),
        tenant_id: row.get("tenant_id"),
