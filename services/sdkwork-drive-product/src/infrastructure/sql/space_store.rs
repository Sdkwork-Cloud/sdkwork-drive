...
impl DriveSpaceStore for SqlSpaceStore {
    async fn insert_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read inserted dr_drive_space failed: {error}"))
        })?;
    ...
    async fn list_spaces(
        ...
        let rows = match (owner_subject_type, owner_subject_id) {
            (Some(owner_type), Some(owner_id)) => {
                ...
                .await
                .map_err(|error| {
                    DriveProductError::Internal(format!("list dr_drive_space by owner failed: {error}"))
                })?
            }
            _ => {
                sqlx::query(
                ...
                .await
                .map_err(|error| {
                    DriveProductError::Internal(format!("list dr_drive_space by tenant failed: {error}"))
                })?
            }
        };
    ...
    async fn get_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("get dr_drive_space failed: {error}"))
        })?;
        let Some(row) = row else {
            return Err(DriveProductError::NotFound("space not found".to_string()));
        };
        map_row_to_space(&row)
    }
    ...
    async fn update_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("update dr_drive_space failed: {error}"))
        })?
        .rows_affected();
        if affected == 0 {
            return Err(DriveProductError::NotFound("space not found".to_string()));
    ...
    async fn delete_space(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("delete dr_drive_space failed: {error}"))
        })?
        .rows_affected();
        if affected == 0 {
            return Err(DriveProductError::NotFound("space not found".to_string()));
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read deleted dr_drive_space failed: {error}"))
        })?;
        map_row_to_space(&row)
    }
}

fn map_row_to_space(row: &AnyRow) -> Result<DriveSpace, DriveProductError> {
