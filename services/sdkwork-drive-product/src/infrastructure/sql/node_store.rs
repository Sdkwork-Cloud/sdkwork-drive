...
impl DriveNodeStore for SqlNodeStore {
    async fn find_active_space_type(
        ...
        tenant_id: &str,
        space_id: &str,
    ) -> Result<Option<DriveSpaceType>, DriveProductError> {
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read dr_drive_space type failed: {error}"))
        })?;
        ...
        space_type_raw
            ...
            .map(|raw| {
                DriveSpaceType::try_from_str(raw).ok_or_else(|| {
                    DriveProductError::Internal(format!("unknown space_type in database: {raw}"))
                })
            })
            .transpose()
    }
    ...
    async fn find_active_node_in_space(
        ...
        space_id: &str,
        node_id: &str,
    ) -> Result<Option<DriveNode>, DriveProductError> {
        let row = sqlx::query(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read dr_drive_node parent failed: {error}"))
        })?;
    ...
    async fn exists_live_name_in_parent(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("check dr_drive_node duplicate failed: {error}"))
        })?;
    ...
    async fn insert_node(&self, new_node: &NewDriveNode) -> Result<DriveNode, DriveProductError> {
        ...
        .map_err(|error| {
            ...
            DriveProductError::Internal(format!("insert dr_drive_node failed: {message}"))
        })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read inserted dr_drive_node failed: {error}"))
        })?;
...
fn drive_node_from_row(row: &AnyRow) -> Result<DriveNode, DriveProductError> {
