...
impl DriveWorkspaceStore for SqlDriveWorkspaceStore {
    async fn find_child_node(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find workspace child node failed: {error}"))
        })?;
    ...
    async fn ensure_node(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("ensure workspace node failed: {error}"))
        })?;
    ...
    async fn ensure_object_ref(
        ...
        Err(DriveProductError::Conflict(
    ...
    async fn mark_node_content_ready(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("mark workspace node ready failed: {error}"))
        })?;
        Ok(())
    }
    ...
    async fn list_children(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("list workspace child nodes failed: {error}"))
        })?;
    ...
    async fn find_node(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find workspace node failed: {error}"))
        })?;
...
impl SqlDriveWorkspaceStore {
    async fn active_object_matches(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("find workspace storage object failed: {error}"))
        })?;
    ...
    async fn replace_active_object_ref_in_transaction(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("retire workspace storage object failed: {error}"))
        })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("ensure workspace storage object failed: {error}"))
        })?;
