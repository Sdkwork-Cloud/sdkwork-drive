...
impl DriveAuditStore for SqlAuditStore {
    async fn append_event(
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("insert dr_drive_audit_event failed: {error}"))
        })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("read inserted dr_drive_audit_event failed: {error}"))
        })?;
    ...
    async fn list_events(
        ...
        let rows = sqlx::query(
            ...
            .await
            .map_err(|error| {
                DriveProductError::Internal(format!("list dr_drive_audit_event failed: {error}"))
            })?;
        ...
        .await
        .map_err(|error| {
            DriveProductError::Internal(format!("count dr_drive_audit_event failed: {error}"))
        })?;
...
fn map_row_to_audit_event(row: &AnyRow) -> Result<DriveAuditEvent, DriveProductError> {
