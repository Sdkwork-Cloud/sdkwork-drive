...
async fn create_rtc_space_is_user_owned_and_unique_per_user() {
    ...
    assert!(matches!(
        duplicate,
        Err(DriveProductError::Conflict(message))
...
async fn create_rtc_space_rejects_non_user_owner() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::Validation(message)) if message.contains("rtc space") && message.contains("user"))
    );
}
...
async fn create_git_repository_space_rejects_non_user_owner() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::Validation(message)) if message.contains("git repository space") && message.contains("user"))
    );
}
...
async fn delete_space_rejects_user_git_repository_space() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::Validation(message)) if message.contains("git repository space") && message.contains("cannot be deleted"))
    );
...
async fn space_service_get_update_and_delete_manage_space_lifecycle() {
    ...
    assert_eq!(
        missing,
        DriveProductError::NotFound("space not found".to_string())
    );
}
...
async fn sql_drive_space_service_exposes_space_operations_without_callers_using_sql_store() {
    ...
    let created = service
        ...
        .await
        .expect("knowledge space should be created through SQL product service");
    ...
    let listed = service
        ...
        .await
        .expect("knowledge spaces should be listed through SQL product service");
    ...
    let found = service
        ...
        .await
        .expect("knowledge space should be loaded through SQL product service");
...
async fn space_service_rejects_invalid_owner_and_operator_before_store_write() {
    ...
    assert_eq!(
        missing_operator,
        DriveProductError::Validation("operator_id is required".to_string())
    );
}
