...
async fn storage_provider_service_rejects_strict_tls_true_for_http_endpoint() {
    ...
    assert_eq!(
        create_error,
        DriveProductError::Validation("strict_tls=true requires an https endpoint".to_string())
    );
...
async fn storage_provider_service_rejects_invalid_endpoint_and_bucket_before_database_write() {
    ...
    assert!(
        matches!(
            invalid_endpoint,
            DriveProductError::Validation(message) if message.contains("endpoint_url")
        ),
    ...
    assert!(
        matches!(
            invalid_bucket,
            DriveProductError::Validation(message) if message.contains("bucket")
        ),
