...
async fn create_folder_rejects_parent_from_another_space() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::NotFound(message)) if message.contains("parent"))
    );
}
...
async fn create_folder_rejects_file_parent() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::Validation(message)) if message.contains("active folder"))
    );
}
...
async fn create_folder_rejects_self_parent() {
    ...
    assert!(
        matches!(result, Err(DriveProductError::Validation(message)) if message.contains("parent_node_id"))
    );
}
...
async fn create_node_rejects_file_at_git_repository_space_root_but_allows_files_inside_repository_directory(
    ...
    let root_file = node_service
        ...
        .await;
    assert!(
        matches!(root_file, Err(DriveProductError::Validation(message)) if message.contains("git repository space root"))
    );
...
) -> sdkwork_drive_product::domain::space::DriveSpace {
