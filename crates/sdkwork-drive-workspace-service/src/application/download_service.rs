...
fn parse_download_token(token: &str) -> Result<ParsedDownloadToken, DriveServiceError> {
...
fn decode_hex(encoded: &str) -> Result<String, DriveServiceError> {
    ...
    while index < encoded.len() {
        ...
        let byte = u8::from_str_radix(pair, 16).map_err(|_| {
            DriveServiceError::Validation("download token node id is invalid".to_string())
        })?;
        bytes.push(byte);
        index += 2;
    }
    ...
    String::from_utf8(bytes)
        .map_err(|_| DriveServiceError::Validation("download token node id is invalid".to_string()))
}
